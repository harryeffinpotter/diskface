...
def path_matches_pattern(path: str, pattern: str) -> bool:
    ...
    # Convert pattern to PurePath for consistent path handling
    pattern = PurePath(pattern)
    path = PurePath(path)
    
    # If pattern is absolute, path must match exactly with glob rules
    if pattern.is_absolute():
        return fnmatch.fnmatch(str(path).lower(), str(pattern).lower())
    
    # For relative patterns (including **), check if any part matches
    ...
    if '**' in pattern_parts:
        ...
        return fnmatch.fnmatch(str(path).lower(), f"*{pattern_str}".lower())
    else:
...
def get_dir_size(path: str, exclusions: Set[str], progress) -> int:
    ...
    try:
        for dirpath, dirnames, filenames in os.walk(path, topdown=True):
            # Filter out excluded directories using pattern matching
...
def create_top_dirs_table(results: List[Tuple[str, int]]) -> Table:
    ...
    # Sort results by size before creating table
...
def analyze_disk_usage(root_path: str, exclusions: Set[str], min_size_mb: float = 100) -> List[Tuple[str, int]]:
            ...
            for dirpath, dirnames, filenames in os.walk(root_path, topdown=True):
                ...
                try:
                    ...
                    current_size = sum(
                        ...
                        and not should_exclude(os.path.join(dirpath, f), exclusions)
                    )
                    
                    if current_size >= min_size_bytes:
                        results.append((dirpath, current_size))
                        # Update the live display with current top 10
                        table = create_top_dirs_table(results)
                        layout = Table.grid()
                        layout.add_row(Panel(progress))
                        layout.add_row(Panel(table))
...
def main():
    ...
    results = analyze_disk_usage('/', exclusions, args.min_size)
